...
public class NewCustomer extends JFrame implements ActionListener{
    ...
    JButton next, cancel;
    String regexno = "^(?=.*\\d).{9,20}$";
    String regexmail = "^(?=.*\\d)([a-z])?([A-Z])?(?=.*[@.]).{8,40}$";
    JLabel lblmeter;
    NewCustomer() {
        setSize(700, 500);
        setLocation(400, 200);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == next) {
            ...
            boolean validNumber = isValid(phone,regexno);
            boolean validemail = isValid(email,regexmail);
            String query1 = "insert into customer values('"+name+"', '"+meter+"', '"+address+"', '"+city+"', '"+state+"', '"+email+"', '"+phone+"')";
            String query2 = "insert into login values('"+meter+"', '', '"+name+"', '', '','','')";
            
            try {
                Conn c = new Conn();
                if(validNumber==true && validemail==true){
